...
/** Stage the railyard demo: three locos (loco + 3 carriages each) spaced around the
...
/** Run the railyard demo until two services complete (or a generous step cap),
 *  accumulating progress, collision/derail, and the no-flip witness. */
function runRailyardDemo(sim: ReturnType<typeof stage>): RunObservations {
...
describe('RailyardDemoController — multi-train loop + serialised yard', () => {
  it('runs collision-free, traverses the curves + split, services the yard with a train→train swap, and never flips a facing', () => {
    const sim = stage();
    const { visited, overlapped, leftRails, worstBottomRotation } = runRailyardDemo(sim);
