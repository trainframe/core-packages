...
(two arcs, split clear of the four exits), a red **operating knob** on the
surround, and the wooden bridge deck spanning wall to wall. Trunk + three rim
stubs sit on the 45° lattice (the spatial open question answered: the disc
snaps like every other piece). `compileLayout` emits one junction marker with
`valid_positions ['stub-a','stub-b','stub-c']` and three switched outbound
