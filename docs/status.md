...
| Experimental devices log                       | shipped | `docs/experimental/` — speculative viability-test device specs that prove a protocol seam without being normative. First entry: [001 vision length station](experimental/001-vision-length-station.md) (CV station that measures + reports `train_length_mm`, proving ADR-023's external-reporting seam). |
