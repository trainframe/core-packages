...
| Core event schemas (13 types)              | shipped | `device_registered`, `tag_observed`, `marker_traversed`, `train_status`, clearance \*, `gate_state_changed`, `switch_state_changed`, `zone_state_changed` (ADR-026), `aspect_changed`, `tag_assignment`, `anomaly`. |
...
| `protocol_version` literal                 | shipped | `0.8.0` exported as `PROTOCOL_VERSION` (0.5.0 added optional `priority` on `device_registered`, ADR-017; 0.6.0 added the `topology_violation` event + retained-clearance `block_reason`, ADR-019; 0.7.0 added the `grant_reverse` command, ADR-022; 0.8.0 added `core.gates_zone` + `zone_state_changed` + `ZoneRetainedState`, ADR-026).              |
