...
| ADR-029                                        | built | Real railyard interior — device-driven shunting. **Entirely a simulator + toy-table concern; core and the wire are unchanged** (the throat handoff of ADR-027 is untouched). Replaces the instantaneous `swapLeadingPair` with a timed 5-step choreography the `VirtualRailyard` runs over a device-private 2D interior (depth from the throat × lane across the slots) while the train is suspended `in_zone`: **enter** a free slot → **decouple** the leading cut (crane decides; no-op if nothing to swap) → **pull-forward** then **reverse-pickup** across into the spares slot, where the waiting cut collides and magnetically auto-couples (sim-only) → **return** to a neutral slot → **inspect** (crane camera reads it) → **release** (`zone_train_released` + ADR-023 length reconcile). The consist changes a cut at a time, never instantly. The toy-table renders train + rake at their interior pose and the gantry follows the live crane pose (`getInteriorState()`), re-rendered each tick. Verified: `virtual-railyard.test.ts` (5-step order, collision pickup, no-swap path), yard-framed screenshot pass (`ui-tests/scripts/railyard-yard-capture.mjs`). Main-line rake overlap fixed first (separable §0): trains report `loco + rake` length so block exclusivity holds the blocks the wagons occupy. |
