...
| Core event schemas (15 types)              | shipped | `device_registered`, `tag_observed`, `marker_traversed`, `train_status`, clearance \*, `gate_state_changed`, `switch_state_changed`, `zone_state_changed` (ADR-026), `zone_train_released` (ADR-027), `train_length_changed` (ADR-023), `aspect_changed`, `tag_assignment`, `anomaly`. |
...
| `protocol_version` literal                 | shipped | `0.9.0` exported as `PROTOCOL_VERSION` (0.5.0 added optional `priority` on `device_registered`, ADR-017; 0.6.0 added the `topology_violation` event + retained-clearance `block_reason`, ADR-019; 0.7.0 added the `grant_reverse` command, ADR-022; 0.8.0 added `core.gates_zone` + `zone_state_changed` + `ZoneRetainedState`, ADR-026; 0.9.0 added `core.reports_length` + `train_length_changed`, ADR-023; 0.10.0 added `core.can_reverse` + `zone_train_released`, ADR-027).              |
...
| `core.gates_zone` built-in                 | shipped | ADR-026. A device owns a capacity-limited territory (railyard) behind one boundary marker and gates admission by its own asserted occupancy (`zone_state_changed { zone_marker_id, capacity, occupancy }`). `onClearanceConsultation` denies entry to the boundary while `occupancy >= capacity`; the existing deny-and-hold/`retryBlockedClearances` cycle admits a held train when a slot frees. **Zero scheduler changes** — pure consultation-voting capability, mirroring `gates_clearance`. Occupancy is a device-asserted, no-oracle fact (carriages are invisible to core, ADR-016) trusted like a length (ADR-023) or tag binding (ADR-007). End-to-end proof: `packages/integration/src/zone-admission.test.ts`. Opaque-interior transit handoff is ADR-026 future work. |
...
| Experiments tray (experimental 001–005)    | shipped | The toybox's third group (`TOYBOX_TRAYS` derives Track/Devices/Experiments from each descriptor's `tray`). 001/002/003/005 are real `TrackPieceType`s in the ADR-024 workshop style; 004 (wedge decoupler) ships no piece — superseded by the railyard's wedge unit. **Vision station**: station plank + grey sensor mast; LED lights while a live train is under the sensor; `ToyHardware.reportVisionLengths` watches `tag_observed` at its marker and asserts `train_length_changed` from `VLS-{id}` with the consist-measured length + hysteresis — the ADR-023 loop closed end-to-end. **Turntable**: skeuomorphic per the real thing (BRIO 33361 / prototype pits) — beech surround, dark recessed pit, steel ring-rail arcs carrying the bridge's end carriages, red operating knob; junction marker compiling to `valid_positions ['stub-a','stub-b','stub-c']` with three switched outbound edges (the N-way proof); the bridge is a separate rotating sub-shape drawn at the confirmed `VirtualSwitch` position; "Spin deck" = a hand on the knob. **Crane**: WORKING cargo — carriages carry the cosmetic `TrackPiece.cargo` slot; "Lift/Place crate" moves a crate between the live trackside stack and the wagon under the hook, bracketed by a withhold/grant pulse on `CRANE-{id}`'s own marker, with nothing cargo-specific on the wire (the 003 negative-space proof, run). The TROLLEY (the arm) is a moving part riding a cantilevered beam: parked over the stack at rest, it slides out over the rail whenever a wagon is under the gantry of a live crane — the doc's trolley-along-the-cross-beam motion, derived state + eased CSS travel. **Lift bridge**: the raised leaf LIFTS — plan-view foreshortening toward the hinge (`LIFT_BRIDGE_FORESHORTEN` scale about the pivot), dark underside end face, longer cast shadow, gap opening beyond the free end; "Raise span" withholds clearance across `M-{id}` via its `BRIDGE-{id}` `VirtualGate`, "Lower" grants — clearance as physical track availability. Moving-part state is read OFF THE SIM each render (never duplicated in React state); `Simulation.getGate`/`getSwitch`, `VirtualGate.isWithholding`/`getWithheldMarkers`, `VirtualSwitch.getPosition`/`setPosition` are the public observers. Mechanical device state SURVIVES track-topology sim rebuilds: `ToyHardware.syncLayout` snapshots switch positions + gate withholds before teardown and the respawned devices re-assert them on the bus (a raised span stays raised; default state stays silent; a deleted piece's state drops). Tests: `pieces.test.ts`, `layout-from-pieces.test.ts`, `toy-hardware.test.ts`, `ToyTable.test.tsx`, `ui-tests/experiments-tray.spec.ts`. |
...
| ADR-023                                        | shipped | Runtime train length changes. Scope deliberately narrowed: the system models a train's **length**, never its composition. Magnetic toy carriages a child swaps by hand are invisible to core (ADR-016 upheld); the only consequence is a length change. **Built (protocol 0.9.0):** `train_length_changed` event + `core.reports_length` capability (producer-authority gated exactly like `core.assigns_tags` — `handleTrainLengthChanged` in scheduler.ts) make `train_length_mm` runtime-mutable and assertable by a device other than the train. The scheduler updates `length_mm` on receipt and re-derives tail occupancy on the next `train_status` (the walk reads head position from the status payload, not stored state → deferred one tick, always safe under ADR-016's hold-don't-guess asymmetry). Explicitly NO identity lifecycle, NO coupling clearance / ADR-011 exception, NO coupling maneuver. Value validation **gate-only** (no oracle; producer trusted like a tag-assigner). First producer: the ADR-026 railyard, reconciling a train's length on exit. |
| ADR-026                                        | accepted (partly impl) | Delegated capacity-territory (zones / railyards). A device owns a capacity-limited region behind one boundary marker and gates admission by its **own asserted occupancy** — a fact core cannot compute, since a slot can be locked by a cut of carriages with no locomotive and carriages are invisible to core (ADR-016). Occupancy joins length (ADR-023) and tags (ADR-007) as a third asserted, capability-gated, no-oracle fact; generalises ADR-011 block exclusivity from a capacity-1 mutex core computes to a capacity-N semaphore the device asserts. **Built:** `core.gates_zone` + `zone_state_changed` + `ZoneRetainedState` (protocol 0.7.0 → 0.8.0, TCF epoch 2 → 3), admission via the consultation veto (zero scheduler changes), scalable `VirtualRailyard`, end-to-end integration test. **Designed, not built:** opaque-interior transit handoff (core suspending management of a tracked train inside + the device issuing its interior movement authority), topology-violation exemption for interior markers, length reconcile on exit (depends on ADR-023). Single-lead interior discipline keeps multi-occupant safety tractable. Resolves the ADR-023 "no shunting" contradiction by relocation: shunting lives in a device, never in core. Proof: experimental [006 railyard](experimental/006-railyard.md). The interior handoff that ADR-026 deferred is now built — see ADR-027. |
| ADR-027                                        | shipped | Zone interior handoff (entry-hold / exit-reclaim), the mechanism ADR-026 deferred. **Built (protocol 0.10.0):** the scheduler keeps a zone-boundary registry (from `zone_state_changed`); **entry** = an admitted train reaching a zone boundary as its route terminus is suspended (`in_zone`, releases all blocks so the throat frees — multi-occupant serialises through the throat); **exit** = the device emits `zone_train_released`, core reclaims the train at the throat and it departs only under ordinary clearance (main-line exclusivity holds — the device never drives a train across the throat). New `core.can_reverse` capability: admission to a zone is **denied** to trains that don't declare it (interior shunting needs reversing), a scheduler-side gate at route planning (pass-through traffic is exempt). The interior itself stays hand-waved (train parks at the throat; device asserts occupancy/length/release). Dropped the topology-violation exemption (dead code without real interior markers). Crossing tests in `scheduler.test.ts`; end-to-end reverse-gate refusal in `zone-admission.test.ts`. Out of scope: `core.can_reverse` informing ADR-022's `grant_reverse`. |
| ADR-029                                        | built | Real railyard interior — device-driven shunting. **Entirely a simulator + toy-table concern; core and the wire are unchanged** (the throat handoff of ADR-027 is untouched). Replaces the instantaneous `swapLeadingPair` with a timed choreography the `VirtualRailyard` runs while the train is suspended `in_zone`, where the **self-propelled train drives itself along the yard's REAL interior rails** (spine → ladder leg → slot, no floating): **enter** a free slot → **decouple** (crane lifts the leading cut; no-op if nothing to swap) → **cross-pull** back onto the spine lead → **cross-set** into the spares slot, coupling the spares on → **inspect** (crane camera reads it, train stays put in the slot) → **release-out** drives back to the throat (`zone_train_released` + ADR-023 length reconcile). The **gantry crane only ever handles CUTS OF CARRIAGES** — lifting the leading cut, carrying it to the spares slot, setting it down as the train takes the spares — never the loco/train; it parks while the train drives. Device is geometry-free (phase + 0..1 progress + slot choices); the toy-table maps each phase onto its centre-line (`railyardInteriorJourney` + `worldHalfPath`). Verified: `virtual-railyard.test.ts` (phase order, cut lift/hold, no-swap path), yard-framed screenshot + video (`ui-tests/scripts/railyard-yard-capture.mjs`, `railyard-single-video.mjs`). Main-line rake overlap fixed first (separable §0): trains report `loco + rake` length so block exclusivity holds the blocks the wagons occupy. |
| Experimental devices log                       | shipped | `docs/experimental/` — speculative viability-test device specs that prove a protocol seam without being normative, judged by "does the API support the action?" not build-realism. Six entries, conceptually grouped into an (unbuilt) **"Experiments" box** in the toy-box tray; each covers API events/data, action goal, visible goal, and toy-box element + animation. [001 vision length station](experimental/001-vision-length-station.md) (CV measures + reports `train_length_mm`); [002 turntable junction](experimental/002-turntable-junction.md) (the switch seam is already N-way — `valid_positions`/`requires_switch_state` string match; a 3-way junction is a piece-geometry task, not a core change); [003 crane cargo station](experimental/003-crane-cargo-station.md) (payload manipulation via dwell+identity+clearance, crate stays out of core like carriages — needs only a cosmetic carriage cargo-slot in the sim); [004 wedge decoupler](experimental/004-wedge-decoupler.md) (ADR-023 decrease direction; isolates the device from the hard *shunting orchestration*, steered by the Inglenook/Timesaver puzzles); [005 lift bridge](experimental/005-lift-bridge.md) (`core.gates_clearance` expresses physical track availability, not just traffic policy); [006 railyard](experimental/006-railyard.md) (**graduated → ADR-026, partly built**: a device owns a capacity-territory and gates admission by asserted occupancy). Held-state across the batch resolved as `core.gates_clearance`, not dwell timers. |
