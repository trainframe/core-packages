...
| Core event schemas (14 types)              | shipped | `device_registered`, `tag_observed`, `marker_traversed`, `train_status`, clearance \*, `gate_state_changed`, `switch_state_changed`, `zone_state_changed` (ADR-026), `train_length_changed` (ADR-023), `aspect_changed`, `tag_assignment`, `anomaly`. |
