...
| HTTP admin API (assignRoute, hold/release, tags)  | shipped | `AdminHttpServer` on a configurable port (default 3000). Endpoints: `/api/health`, `/api/state`, `/api/trains/:id/route`, `/api/trains/:id/revoke_clearance` (routed through `Server.revokeClearance` so the scheduler's view of who owns which block stays in sync with the train's behavior), `/api/gates/:id/hold`, `/api/gates/:id/release`, `/api/tags`. CLI: `--http-port`. No auth (LAN/localhost). ADR-008. |
