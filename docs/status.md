...
| ADR-023                                        | proposed | Coupling/decoupling design (NOT yet implemented). Runtime capability-gated scalar length change (`consist_changed` event + `core.reports_consist_change` capability, mirroring `core.assigns_tags` enforcement) so a robotic decoupler — not only the train — can assert a new `train_length_mm`; scheduler re-derives occupancy. Identity is slave/resume of pre-existing motion-device identities (never mint from nothing); on couple, occupancy **transfers, not frees** (unlike the disconnect path). Physical coupling uses a tightly-scoped, mediator-requested, revocable coupling-clearance exception to ADR-011 (two named trains, one section, crawl speed); decouple needs none. Maneuvering composes from ADR-022 reverse + routes. Carriages stay out of core (ADR-016 deferral reaffirmed). Three decisions flagged for review. |
...
- Coupling/decoupling: trains as multi-vehicle compositions — design drafted in ADR-023 (**Proposed**, awaiting review of three flagged decisions); not yet accepted or implemented.
...
1. **Coupling/decoupling: trains as multi-vehicle compositions** — design drafted in **ADR-023 (Proposed)**: runtime capability-gated scalar length change (`consist_changed` + `core.reports_consist_change`), slave/resume identity lifecycle (never mint from nothing), and a controlled coupling-clearance exception to ADR-011. Three decisions flagged for review before Accepted (couple-survivor authority, the exclusivity exception's safety story, the length-value trust boundary). Phase 1 (runtime length + occupancy re-derivation) is foundational and low-risk once accepted; carriages stay a simulator/visualiser detail (ADR-016 deferral reaffirmed).
