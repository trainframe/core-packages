...
| Experiments tray (experimental 001–005)    | shipped | The toybox's third group (`TOYBOX_TRAYS` derives Track/Devices/Experiments from each descriptor's `tray`). All five viability-test pieces are real `TrackPieceType`s in the ADR-024 workshop style: **vision station** (station plank + grey sensor mast; LED lights while a live train is under the sensor; `ToyHardware.reportVisionLengths` watches `tag_observed` at its marker and asserts `train_length_changed` from `VLS-{id}` with the consist-measured length + hysteresis — the ADR-023 loop closed end-to-end), **turntable** (beech disc, junction marker compiling to `valid_positions ['stub-a','stub-b','stub-c']` with three switched outbound edges — the N-way proof; the bridge deck is a separate rotating sub-shape drawn at the confirmed `VirtualSwitch` position, eased; "Spin deck" ActionBar affordance = a hand on the toy), **crane** (station + grey gantry/trolley + pop crates; `CRANE-{id}` gate identity; cargo slots remain unbuilt per the doc), **decoupler** (wire device `DEC-{id}` declaring reports_length + gates_clearance, gate-backed; wedge static — shunting orchestration deliberately parked), **lift bridge** (hinged span as a separate tilting sub-shape over a dark gap; "Raise span" withholds clearance across `M-{id}` via its `BRIDGE-{id}` `VirtualGate`, "Lower" grants — clearance as physical track availability). Moving-part state is read OFF THE SIM each render (never duplicated in React state); `Simulation.getGate`/`getSwitch`, `VirtualGate.isWithholding`, `VirtualSwitch.getPosition`/`setPosition` are the new public observers. Tests: `pieces.test.ts`, `layout-from-pieces.test.ts`, `toy-hardware.test.ts`, `ToyTable.test.tsx`, `ui-tests/experiments-tray.spec.ts`. |
