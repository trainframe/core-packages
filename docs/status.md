...
| `protocol_version` literal                 | shipped | `0.4.0` exported as `PROTOCOL_VERSION` (bumped for the documented devices retained-state shape + `train_length_mm`, ADR-016).              |
...
| ------------------------------------------ | :----: | ----------------------------------------------------------------------------- |
| Static-shell deployment                    | shipped | Pages-deployed, broker URL via localStorage.                                   |
| Toy table (toybox + canvas + scan-box)     | shipped | `ToyTable` component. Toybox holds track piece types (straight, curve, junction, station, terminus, crossing, ramp) and devices (train, gate, carriage). Click-to-arm + click-to-place. Pieces start inert; only scanning makes them live on the bus. |
| Wooden-track aesthetic                      | shipped | ADR-017. Pieces render as consistent beech-wood planks: a wood-gradient body, twin routed rail grooves derived from the SAME centre-line a train rides (`getPieceShape` → `{ svgPath, grooves, features, width, height }`), rim-light + contact shadow, and warm functional tints (`PIECE_TINT`: station/terminus/ramp; junction/crossing read from their silhouette). Devices get characterful top-down bodies (loco, carriage, gate boom). Selection/overlap are a colour glow (no seam on multi-plank pieces). Topology is unchanged — purely visual. The whole app wears a warm "workshop" theme (wood-desk surround, paper card, rounded display font). |
